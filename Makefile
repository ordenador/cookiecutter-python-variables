...
clusters_swarm: venv ## generate file clusters_swarm.tf depends of clusters_swarm.yaml
	./clusters_swarm.py

jenkins_credentials_apply: requirements ## update or create credentials in Jenkins
	./jenkins_credentials.py --apply

jenkins_credentials_check: requirements ## check if exists paths only
	./jenkins_credentials.py

ldap_assign: requirements ## check if exists paths only
	./ldap_assign.py
